...
function getTx(txid, res) {
	async.waterfall([
      ...
      try {
        ...
        var exclusive = {};
        if (txsResult[0].type == "MinerTransaction") {
          var sqlExclusiveTx = "SELECT * FROM miner_transaction WHERE txid=?";
          var exclusiveTx = connection.query(sqlExclusiveTx, [txid]);
          exclusive = exclusiveTx[0];
        } else if (txsResult[0].type == "IssueTransaction") {
          var sqlExclusiveTx = "SELECT * FROM issue_transaction WHERE txid=?";
          var exclusiveTx = connection.query(sqlExclusiveTx, [txid]);
          exclusive = exclusiveTx[0];
        } else if (txsResult[0].type == "ClaimTransaction") {
          var sqlExclusiveTx = "SELECT * FROM claim_transaction WHERE txid=?";
          var exclusiveTx = connection.query(sqlExclusiveTx, [txid]);
          exclusive = exclusiveTx[0];
        } else if (txsResult[0].type == "EnrollmentTransaction") {
          var sqlExclusiveTx = "SELECT * FROM enrollment_transaction WHERE txid=?";
          var exclusiveTx = connection.query(sqlExclusiveTx, [txid]);
          exclusive = exclusiveTx[0];
        } else if (txsResult[0].type == "RegisterTransaction") {
          var sqlExclusiveTx = "SELECT * FROM register_transaction WHERE txid=?";
          var exclusiveTx = connection.query(sqlExclusiveTx, [txid]);
          exclusive = exclusiveTx[0];
        } else if (txsResult[0].type == "ContractTransaction") {
          var sqlExclusiveTx = "SELECT * FROM contract_transaction WHERE txid=?";
          var exclusiveTx = connection.query(sqlExclusiveTx, [txid]);
          exclusive = exclusiveTx[0];
        } else if (txsResult[0].type == "AnonymousContractTransaction") {
          var sqlExclusiveTx = "SELECT * FROM anonymous_contract_transaction WHERE txid=?";
          var exclusiveTx = connection.query(sqlExclusiveTx, [txid]);
          exclusive = exclusiveTx[0];
        } else if (txsResult[0].type == "PublishTransaction") {
          var sqlExclusiveTx = "SELECT * FROM publish_transaction WHERE txid=?";
          var exclusiveTx = connection.query(sqlExclusiveTx, [txid]);
          exclusive = exclusiveTx[0];
        } else if (txsResult[0].type == "InvocationTransaction") {
